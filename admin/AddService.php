...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    if (empty($name)) {
        $name_error = 'Please insert service\'s name';
    } else {
        $name_error = '';
    }

    if (empty($detail)) {
        $detail_error = 'Please insert service\'s details';
    } else {
        $detail_error = '';
    }

    if (empty($price)) {
        $price_error = 'Please insert service\'s price';
    } else {
        $price_error = '';
    }


    if(isset($_POST['submit']) && isset($_FILES['upload']) && empty($name_error) && empty($detail_error) && empty($price_error))
        {
            ...
            if($error === 0)
            {
                ...
                if ($img_size > 3145728) {
                    ...
                    header("Location: AddService.php?error=$error_message");
                }
                else
                {
                   ...
                   //image format allowed are jpg, jpeg and png only, no jfif
                    ...
                    $query_image2 = "INSERT INTO service(servicepic, servicename, servicedetail, price) VALUES('$upload_path','$name', '$detail', '$price')";
  ...
  <div class="transaction">
    ...
    <form method="post" enctype="multipart/form-data" auto_complete="off" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) ;?>" >
        <div>
            ...
            <input type="text" class="serviceField" id="servicename" name="servicename" placeholder="Service Name" value="<?php echo ($name);?>"></input>
        ...
        <div>
            ...
            <input type="text" class="serviceField" id="servicedetail" name="servicedetail" placeholder="Service Details" value="<?php echo ($detail);?>"></input>
        ...
        <div>
            ...
            <input type="text" class="serviceField" id="price" name="price" placeholder="Service Price" value="<?php echo ($price);?>"></input>
